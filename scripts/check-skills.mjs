...
for (const name of skillNames) {
  ...
  const skillLineCount = skillText.trimEnd().split(/\r?\n/).length;
  const skillWordCount = skillText.match(/\S+/g)?.length ?? 0;
  if (skillLineCount >= skillLineLimit) {
    fail(`${relative(skillFile)} must stay below ${skillLineLimit} lines`);
  }
  if (skillWordCount >= skillWordLimit) {
    fail(`${relative(skillFile)} must stay below ${skillWordLimit} words`);
  }
  const hotSkillLineLimit = hotSkillLineLimits.get(name);
  if (hotSkillLineLimit && skillLineCount > hotSkillLineLimit) {
