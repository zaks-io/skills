...
This packaged copy is named `zaks-neon-postgres`. Upstream Neon install examples
may still refer to the source skill as `neon-postgres`.
...
This installs the Neon extension (for Cursor/VS Code) or MCP server (for other agents), creates an API key, and, in a default upstream setup, adds the source `neon-postgres` agent skill to the project. When using this repository, keep the checked-in skill name as `zaks-neon-postgres`.
